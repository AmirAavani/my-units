...
type
  ...
  TRequestHandler = procedure(const ServiceName, MethodName: AnsiString;
    RequestData: TByteArray; out ResponseData: TByteArray;
    out StatusCode: Integer; out StatusMessage: AnsiString) of object;
  ...
  private
    ...
    function ParseHTTPRequest(ClientSocket: TSocket;
      out Method, Path, ContentType: AnsiString; out ContentLength: Integer;
      out BodyData: TByteArray): Boolean;
    function ReadRequestBody(ClientSocket: TSocket; ContentLength: Integer): TByteArray;
    procedure SendHTTPResponse(ClientSocket: TSocket; StatusCode: Integer;
      const StatusMessage: AnsiString; const ResponseData: TByteArray);
...
begin
  ...
  if not ParseHTTPRequest(ClientSocket, Method, Path, ContentType,
    ContentLength, InitialBodyData) then
  begin
  ...
  begin
    ...
    begin
      ...
      Move(RequestData[0], FramedRequestData[Length(InitialBodyData)],
        Length(RequestData));
    end
    else
...
procedure THTTPGRPCServerTransport.SendHTTPResponse(ClientSocket: TSocket;
  StatusCode: Integer; const StatusMessage: AnsiString; const ResponseData: TByteArray);
var
  Response: AnsiString;
...
begin
  ...
  TrailerText := Format('grpc-status: %d'#13#10'grpc-message: %s'#13#10, [0, 'OK']);
...
function THTTPGRPCServerTransport.UnframeMessage(
  const FramedData: TByteArray): TByteArray;
var
  MessageLen: UInt32;
  i: Integer;
begin
  ...
  MessageLen := (UInt32(FramedData[1]) shl 24) or
    ...
    (UInt32(FramedData[3]) shl 8) or UInt32(FramedData[4]);
