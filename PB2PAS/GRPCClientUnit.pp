...
{ ============================================================================
  ...
  This unit implements a gRPC-Web compatible client that can call:
  - Pascal gRPC-Web servers (generated by PB2PAS)
  - Go gRPC-Web servers (with grpc-web wrapper)
  - Any gRPC-Web compatible service
  
  Protocol: gRPC-Web (application/grpc-web+proto)
  Transport: HTTP/1.1 with 5-byte frame headers
  
  To call standard gRPC servers (HTTP/2), you need:
  1. A gRPC-Web proxy between client and server
  2. Or enable gRPC-Web support on the server
...
type
  ...
  public
    ...
    // Simple constructor for basic usage (uses gRPC-Web protocol)
...
begin
  ...
  // Use gRPC-Web protocol (HTTP/1.1 with binary framing)
  FTransport := THTTPGRPCTransport.Create(FConfig, True);
  FLastStatusCode := 0;
  FLastStatusMessage := '';
  FRaiseOnError := False;
end;
